...
class AsyncClient:
    ...
    Resource methods must be awaited:

        product = await client.products.get("pro_123")
        collection = await client.products.list()

    Calls are fully dynamic: the sync resource clients call self.client._get /
    _post / _patch / _delete, which are async def here, so the call returns a
    coroutine that the caller awaits.  Static typing is provided by
    AsyncClient.pyi and the generated Async*Client.pyi stubs alongside each
    resource.
    """
